...
public:
    ...
    void deleteColumns(const std::vector<size_t> & columns);
    ...
    const std::string & attrName(size_t col) const
    {
        return m_attr_name[col];
    }

    // Returns the name of the specified value
    const std::string & attrValue(size_t attr, size_t val)
    {
        return m_enum_to_str[attr][val];
    }

    // Returns a reference to the specified row
    std::vector<double> & row(size_t index)
    {
        return m_data[index];
    }
    ...
    // Returns the number of values associated with the specified attribute 
    // (or column) 0=continuous, 2=binary, 3=trinary, etc.
    size_t valueCount(size_t attr) const
    {
        return m_enum_to_str[attr].size();
    }
    ...
    void checkCompatibility(Matrix & that) const;
