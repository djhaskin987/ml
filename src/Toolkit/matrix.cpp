...
void Matrix::deleteColumns(const vector<size_t> & columns)
{
    vector<size_t> ToDelete = columns;
    sort(ToDelete.begin(), ToDelete.end(), greater<size_t>());
    vector<size_t>::iterator chop = unique(ToDelete.begin(), ToDelete.end());
    ToDelete.erase(chop, ToDelete.end()); 
    size_t c = cols();
    // metadata
    for (vector<size_t>::iterator iter = ToDelete.begin();
           iter != ToDelete.end();
           iter++)
    {
        m_attr_name.erase(m_attr_name.begin()+*iter); 
        m_str_to_enum.erase(m_str_to_enum.begin()+*iter); 
        m_enum_to_str.erase(m_enum_to_str.begin()+*iter); 
    }
    for (int i = 0; i < rows(); i++)
    {
        for (vector<size_t>::iterator iter = ToDelete.begin();
               ...
               iter++)
        {
            m_data[i].erase(m_data[i].begin()+*iter);
        }
    }
}
...
double Matrix::columnMean(size_t col) const
{
    if (rows() < 0)
    {
        ThrowError("cannot take the mean of an empty matrix.");
    }

    if (valueCount(col) > 0)
    {
        ThrowError("nominal column");
    }
    double sum = 0.0;
    size_t count = 0;
    ...
    for(it = m_data.begin(); it != m_data.end(); it++)
    {
        ...
        assert(!std::isnan((double)val) &&
                val != numeric_limits<double>::infinity() &&
                val != -numeric_limits<double>::infinity());
    }
    assert(!std::isnan((double)sum) &&
            ...
            while ( !inputFile.eof() )
            {
                ...
                if ( line.find ( "%" ) != 0 && line != "" )
                {
                    ...
                    for ( size_t i = 0; i < attrCount; i++ )
                    {
                        ...
                        string val = line.substr(line.find_first_not_of(" " ),
                                line.find_first_of ( "\t,\n\r" ) );
...
void Matrix::checkCompatibility(Matrix& that) const
{
    size_t c = cols();
    if(that.cols() != c)
